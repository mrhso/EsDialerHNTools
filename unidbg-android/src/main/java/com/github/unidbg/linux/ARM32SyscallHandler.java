...
public class ARM32SyscallHandler extends AndroidSyscallHandler {
	...
	public void hook(Backend backend, int intno, int swi, Object user) {
		...
		try {
			if (swi == 0 && NR == 0
					&& (backend.reg_read(ArmConst.UC_ARM_REG_R5).intValue()) == Svc.POST_CALLBACK_SYSCALL_NUMBER) { // postCallback
			...
			if (swi == 0 && NR == 0
					&& (backend.reg_read(ArmConst.UC_ARM_REG_R5).intValue()) == Svc.PRE_CALLBACK_SYSCALL_NUMBER) { // preCallback
				int number = backend.reg_read(ArmConst.UC_ARM_REG_R4).intValue();
				Svc svc = svcMemory.getSvc(number);
				if (svc != null) {
		...
		log.warn("handleInterrupt intno=" + intno + ", NR=" + NR + ", svcNumber=0x" + Integer.toHexString(swi) + ", PC="
				+ pc + ", LR=" + emulator.getContext().getLRPointer() + ", syscall=" + syscall, exception);
	...
	private int mremap(Emulator<?> emulator) {
		...
		final long address;
		if (fixed) {
			address = memory.mmap2(new_address.toUIntPeer(), new_size, prot,
	...
	private int pthread_clone(Emulator<?> emulator) {
		...
		if (threadDispatcherEnabled) {
			...
			emulator.getThreadDispatcher()
					.addThread(new KitKatThread(threadId, emulator.getReturnAddress(), child_stack, fn, arg));
			return threadId;
		}
	...
	private int newselect(Backend backend, Emulator<?> emulator) {
		...
		throw new AbstractMethodError("newselect nfds=" + nfds + ", readfds=null, writefds=" + writefds
				+ ", exceptfds=null, timeout=" + timeout);
	}
	...
	protected int pselect6(Emulator<?> emulator) {
		...
		if (log.isDebugEnabled()) {
			log.debug("pselect6 nfds=" + nfds + ", readfds=" + readfds + ", writefds=" + writefds + ", exceptfds="
					+ exceptfds + ", timeout=" + timeout + ", LR=" + context.getLRPointer());
			if (readfds != null) {
				byte[] data = readfds.getByteArray(0, size);
				Inspector.inspect(data, "readfds");
			}
		...
		throw new AbstractMethodError("pselect6 nfds=" + nfds + ", readfds=null, writefds=" + writefds
				+ ", exceptfds=null, timeout=" + timeout + ", LR=" + context.getLRPointer());
	}
	...
	private int getsockopt(Backend backend, Emulator<?> emulator) {
		...
		if (log.isDebugEnabled()) {
			log.debug("getsockopt sockfd=" + sockfd + ", level=" + level + ", optname=" + optname + ", optval=" + optval
					+ ", optlen=" + optlen + ", from=" + UnidbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR));
		}
	...
	private int socket(Backend backend, Emulator<?> emulator) {
		...
		if (log.isDebugEnabled()) {
			System.out.println("socket domain=" + domain + ", type=" + type + ", protocol=" + protocol);
		}
	...
	private int munmap(Backend backend, Emulator<?> emulator) {
		...
		if (log.isDebugEnabled()) {
			log.debug("munmap start=0x" + Long.toHexString(start) + ", length=" + length + ", offset="
...
//		if (log.isDebugEnabled()) {
			log.info("mprotect address=0x" + Long.toHexString(address) + ", alignedAddress=0x"
...
//		}
	...
	private int mmap2(Backend backend, Emulator<?> emulator) {
		...
		if (log.isDebugEnabled() || warning) {
			String msg = "mmap2 start=0x" + Long.toHexString(start) + ", length=" + length + ", prot=0x"
					...
					+ offset + ", from=" + UnidbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR);
			if (warning) {
				log.warn(msg);
			} else {
				log.debug(msg);
	...
	private int open(Emulator<AndroidFileIO> emulator) {
		...
		String msg = "open pathname=" + pathname + ", oflags=0x" + Integer.toHexString(oflags) + ", mode="
				+ Integer.toHexString(mode) + ", from=" + UnidbgPointer.register(emulator, ArmConst.UC_ARM_REG_LR);
		if (log.isDebugEnabled()) {
			log.debug(msg);
		}
		int fd = open(emulator, pathname, oflags);
	...
	private int ioctl(Emulator<?> emulator) {
		...
		if (log.isDebugEnabled()) {
			System.out.println("ioctl fd=" + fd + ", request=0x" + Long.toHexString(request) + ", argp=0x"
	...
	private int pread64(Emulator<?> emulator) {
		...
		long offset = context.getIntByReg(ArmConst.UC_ARM_REG_R4)
				| ((long) context.getIntByReg(ArmConst.UC_ARM_REG_R5) << 32L);
